...
export const cofCurrencyBuy = (input: IInput, data: IExchange[]): string => data.filter((item: IExchange) => item.ccy === input.select)[0]?.buy;
export const cofCurrencySale = (input: IInput, data: IExchange[]): string => data.filter((item: IExchange) => item.ccy === input.select)[0]?.sale;
